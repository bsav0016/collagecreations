...
function AdminTicket({ customOrder }: AdminTicketProps): React.ReactElement {
  ...
  const fields: FormFieldConfig[] = ticketData ? [
  ...
  const handleFieldChange = (fieldName: string, checkbox: boolean) => (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>): void => {
    setTicketData(prev => {
      ...
      if (checkbox) {
        ...
        updatedTicketData.updateField(fieldName, target.checked);
        if (fieldName === 'inProgress' && target.checked) {
          updatedTicketData.updateField('resolved', false);
        }
        else if (fieldName === 'resolved' && target.checked) {
