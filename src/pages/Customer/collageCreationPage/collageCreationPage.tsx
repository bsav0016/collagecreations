...
export function CollageCreation({ isAdmin = false }: CollageCreationPageProps) {
  ...
  const currentStep: CollageCreationStep =
    step &&
    Object.values(CollageCreationStep).includes(
  ...
  const goToNextStep = (nextStep: CollageCreationStep) =>
    navigate(`${baseCollageUrlExt}${nextStep}`);
  ...
  const [outputSymbol, setOutputSymbol] = useState<SymbolOption | null>(
    defaultOutputSymbol,
  );
  const [smallImageSize, setSmallImageSize] = useState<SmallImageSize>(
    defaultSmallImageSize,
  );
  const [lightDarkArray, setLightDarkArray] = useState<boolean[][]>(
    defaultLightDarkArray,
  );
      ...
      : currentStep === CollageCreationStep.SelectOutputSizeStep
        ...
        : currentStep === CollageCreationStep.SelectOutputStep
          ...
          : currentStep === CollageCreationStep.SelectSmallSizeStep
            ? "Select Small Image Sizes"
            : currentStep === CollageCreationStep.PreviewStep
              ? "Light / Dark Image Preview"
              : currentStep === CollageCreationStep.SelectImagesStep
                ? "Select Small Images"
                : currentStep === CollageCreationStep.CreateCollageStep
                  ? "Final Step!"
                  : null;

  const subtitle: string | null =
    currentStep === CollageCreationStep.SelectOutputSizeStep
      ? "This is the size, in inches, that your collage will print to (width x height)"
      : currentStep === CollageCreationStep.SelectSmallSizeStep
        ? "This is the size of the images that make up your collage (as compared to a US quarter)"
        : currentStep === CollageCreationStep.PreviewStep
          ? "This is an outline of how your collage will look. Click on any box you would like to be light instead of dark or vice versa"
          : currentStep === CollageCreationStep.SelectImagesStep
            ? "Please select at least 5 images. The more images you include, the more diversity your collage will have"
            : currentStep === CollageCreationStep.CreateCollageStep
              ? "Please give the server up to 2 minutes to process your images"
              : null;
