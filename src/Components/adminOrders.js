    ...
    <div className="App">
      <header className="App-header" style={{ flexDirection: 'column'}}>
        ...
        <div>
          ...
          />
          <button style={{ margin: 5 }} className='custom-file-upload' onClick={handleGoClick}>Go</button>
        </div>

        <div>
          <h2>Invalid Orders</h2>
          {invalidOrders.map(order => (
            <button style={{ margin: 5 }} className='custom-file-upload' key={order.id} onClick={() => handleOrderClick(order.id)}>
              Order {order.id}
            </button>
          ))}
        </div>
