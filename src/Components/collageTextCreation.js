...
function CollageTextCreation() {
  ...
  function navigateToPreview(output, watermark_output) {
    navigate('/preview/', {
      state: { output: output, watermark_output: watermark_output }
    });
  }
  ...
  return (
    <div>
      ...
      <div className="App">
        ...
        <div>
          <select value={size} onChange={(e) => setSize(e.target.value)} className="drop-down">
        ...
        <div className="add-image-container">
          <input style={{fontSize: 20}} value={text} type="text" placeholder="Enter Text" onChange={(e) => setText(e.target.value)} disabled={isCreatingImage} maxLength={20} />
        </div>
        ...
        <div>
          <button onClick={createImage} className='custom-file-upload' disabled={isCreatingImage}>
